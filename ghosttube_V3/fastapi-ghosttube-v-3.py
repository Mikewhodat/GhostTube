...
def get_video_title(url: str) -> str:
    """Extract title with timeout and retry"""
    for attempt in range(MAX_RETRIES):
        try:
            ...
            log.warning(f"Title fetch timeout for {url}, retry {attempt+1}/{MAX_RETRIES}")
            time.sleep(INITIAL_BACKOFF * (2 ** attempt))
        except Exception as e:
            log.warning(f"Title fetch error for {url}: {e}, retry {attempt+1}/{MAX_RETRIES}")
            time.sleep(INITIAL_BACKOFF * (2 ** attempt))
    return 'Unknown'
...
def get_tor_ip(force_refresh: bool = False) -> str:
    """Get Tor IP with caching and retries"""
    global tor_ip_cache
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            log.warning(f"Tor IP fetch failed (attempt {attempt+1}/{MAX_RETRIES}): {e}")
            time.sleep(INITIAL_BACKOFF * (2 ** attempt))
    return 'Unknown'
...
def rotate_tor() -> bool:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            log.warning(f"Rotate attempt {attempt + 1}/{MAX_RETRIES} failed: {e}")
            time.sleep(INITIAL_BACKOFF * (2 ** attempt))
    
    log.error("Failed to rotate Tor after retries")
    return False
...
def _search_ytdlp(query: str, max_results: int) -> List[str]:
    ...
    cmd = [
        ...
        '--flat-playlist',
        f'ytsearch{max_results}:{query}'  # ytsearchN:query for N results
    ]
    try:
        ...
        try:
            ...
            if '429' in error_msg or 'Too Many Requests' in error_msg:
                ...
                if rate_limiter.ip_rotation_needed:
                    log.warning("Rate limit threshold reached, rotating IP")
                    rotate_tor()
